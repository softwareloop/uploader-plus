...
public interface Log4jHelper
{
    ...
    /**
     * Retrieves a specific appender in the log4j configuration.
     *
     * @param appenderName
     *     the name of the appender to retrieve
     * @param loggerName
     *     the optional name of the logger to which the appender is registered
     * @return the appender
     */
    Object getAppender(String appenderName, String loggerName);
    ...
    /**
     * Creates a temporary file appender to snapshot log output from all current loggers and returns the unique name of the appender for
     * future reference. The temporary appender will be automatically deregistered after an amount of time configured in the system
     * configuration.
     *
     * @return the UUID of the appender
     */
    ...
    /**
     * Closes down a temporary file appender registered to snapshot log output from loggers and returns the path to the log output file.
     *
     * @param uuid
     *     the UUID of the appender
     * @return the path to the log output file or {@code Optional#empty() no path} if there is no appender registered with the provided name
     */
    Optional<Path> closeSnapshotAppender(String uuid);
}
